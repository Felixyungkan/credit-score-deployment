...
# Diganti dari request dari fastAPI jadi lagsung pake sagemaker
...
def make_prediction(features):
    ...
    payload = {
        "instances": [[
            ...
            features["Num_Bank_Accounts"],
            features["Num_Credit_Card"],
            features["Interest_Rate"],
            features["Num_of_Loan"],
            features["Delay_from_due_date"],
            features["Num_of_Delayed_Payment"],
            features["Changed_Credit_Limit"],
            features["Num_Credit_Inquiries"],
            features["Credit_Mix"],
            features["Outstanding_Debt"],
            features["Credit_Utilization_Ratio"],
            features["Credit_History_Age"],
            features["Payment_of_Min_Amount"],
            features["Total_EMI_per_month"],
            features["Amount_invested_monthly"],
            features["Payment_Behaviour"],
            features["Monthly_Balance"],
            features["Num_Loan_Types"]
        ]]
    }
    ...
    if st.button("Make Prediction"):
        ...
        try:
            ...
            if "labels" in result:
                ...
                st.success(
                    f"Predicted Credit Score: {result['labels'][0]}"
                )

            elif "prediction" in result:

                st.success(
                    f"Predicted Credit Score: {result['prediction'][0]}"
                )
